...
# Load all three EEG files
# Load the 200-500 ms dataset 
EEGdata_200_500 <- read.csv("/Users/hoangmy/Desktop/thesis workspace/data/EEG data/Fz_200_500.txt", header = TRUE, sep = "")
# Load the 300-600 ms dataset
EEGdata_300_600 <- read.csv("/Users/hoangmy/Desktop/thesis workspace/data/EEG data/Fz_300_600.txt", header = TRUE, sep = "")
# Load the 400-900 ms dataset
EEGdata_400_900 <- read.csv("/Users/hoangmy/Desktop/thesis workspace/data/EEG data/Fz_400_900.txt", header = TRUE, sep = "")

# Function to reshape the data
reshape_eeg_data <- function(data, time_window) {
  data_long <- data %>%
    pivot_longer(
      cols = -File,  # Exclude the "File" column
      names_to = c("Electrode", "Condition"),  # Extract Electrode and Condition
      names_sep = "\\.",  # Split by the "." separator
      values_to = "Voltage"
    ) %>%
    mutate(Time_Window = time_window)  # Add the Time_Window column
  return(data_long)
}

# Reshape each dataset
EEGdata_200_500_long <- reshape_eeg_data(EEGdata_200_500, "200-500 ms")
EEGdata_300_600_long <- reshape_eeg_data(EEGdata_300_600, "300-600 ms")
EEGdata_400_900_long <- reshape_eeg_data(EEGdata_400_900, "400-900 ms")

# Combine the reshaped datasets
EEG_combined <- bind_rows(
  EEGdata_200_500_long,
  EEGdata_300_600_long,
  EEGdata_400_900_long
)

# Save the combined dataset
write.csv(EEG_combined, "/Users/hoangmy/Desktop/thesis workspace/data/EEG_combined.csv", row.names = FALSE)
...
merged_EEG_dataset <- read_csv("/Users/hoangmy/Desktop/thesis workspace/data/EEG_combined.csv")
# Map the Condition values (AverageCorrect_2 and AverageViolation_2) to grammaticality with values Grammatical and Violation.
merged_EEG_dataset <- merged_EEG_dataset %>%
  mutate(
    grammaticality = case_when(
      Condition == "AverageCorrect_2" ~ "Grammatical",
      Condition == "AverageViolation_2" ~ "Violation",
      TRUE ~ NA_character_  # Handle unexpected values, if any
    )
  )

#Group and Calculate Mean Voltage
mean_amplitude_data <- merged_EEG_dataset %>%
  group_by(File, grammaticality) %>%  # Group by `File` and `grammaticality`
  summarize(
    mean_amplitude = mean(Voltage, na.rm = TRUE),  # Calculate mean voltage
    .groups = "drop"  # Ungroup after summarizing
  )
# Reshape the data
mean_amplitude_wide <- mean_amplitude_data %>%
  pivot_wider(
    names_from = grammaticality,
    values_from = mean_amplitude,
    names_prefix = "mean_"
  )
...
# Get the p-value for the averages differces
# Perform Welch's t-test
t_test_result <- t.test(accuracy ~ grammaticality, data = long_data)
print(t_test_result)
...
# (ERROR) Calculate the difference wave (violation minus grammatical)
merged_data <- merged_data %>%
  mutate(amplitude_difference = mean_violation_amplitude - mean_correct_amplitude)

### (ERROR) Visualize distribution ### (xem co can  chay cai ni khong)
ggplot(merged_data, aes(x = amplitude_difference)) +
  ...
  theme_minimal()
##########################
# Load the 200-500 ms dataset with a specified delimiter
EEGdata_200_500 <- read.csv("/Users/hoangmy/Desktop/thesis workspace/data/EEG data/Fz_200_500.txt", header = TRUE, sep = "")
# Load the 300-600 ms dataset
EEGdata_300_600 <- read.csv("/Users/hoangmy/Desktop/thesis workspace/data/EEG data/Fz_300_600.txt", header = TRUE, sep = "")
# Load the 400-900 ms dataset
...
# Add a time_window column and standardize column names for each dataset
EEGdata_200_500 <- EEGdata_200_500 %>%
  mutate(time_window = "200-500 ms") %>%
  rename_with(~ gsub("\\.+", "_", .))  # Standardize column names

EEGdata_300_600 <- EEGdata_300_600 %>%
  mutate(time_window = "300-600 ms") %>%
  rename_with(~ gsub("\\.+", "_", .))  # Standardize column names

EEGdata_400_900 <- EEGdata_400_900 %>%
  mutate(time_window = "400-900 ms") %>%
  rename_with(~ gsub("\\.+", "_", .))  # Standardize column names
...
# Save the combined dataset for verification
write.csv(EEG_combined, "/Users/hoangmy/Desktop/thesis workspace/data/EEG_combined.csv", row.names = FALSE)

# Step 1: Filter Session 2 Data
session2_data <- merged_data %>%
  filter(!is.na(mean_amplitude)) %>%  # Keep rows with EEG data
  select(
    home_IDs, lab_IDs,  # Participant information
    grammaticality,      # Grammaticality condition
    mean_amplitude,      # EEG data
    `correct_responses_gender violation`, 
    `correct_responses_grammatical`, 
    `correct_responses_number violation`,  # Correct responses
    `total_responses_gender violation`, 
    `total_responses_grammatical`, 
    `total_responses_number violation`,    # Total responses
    `accuracy_percentage_gender violation`, 
    `accuracy_percentage_grammatical`,
    `accuracy_percentage_number violation` # Accuracy percentages
  )

# Step 2: Aggregate Data for Analysis
aggregated_data <- session2_data %>%
  group_by(grammaticality, lab_IDs) %>%  # Group by condition and region
  summarise(
    mean_amplitude = mean(mean_amplitude, na.rm = TRUE),  # Average amplitude
    SE = if (n() > 1) sd(mean_amplitude, na.rm = TRUE) / sqrt(n()) else NA_real_,  # Standard error
    CI_lower = if (n() > 1) mean_amplitude - 1.96 * SE else NA_real_,  # Lower CI
    CI_upper = if (n() > 1) mean_amplitude + 1.96 * SE else NA_real_,  # Upper CI
    .groups = 'drop'
  )
...
plot_data <- aggregated_data %>%
  mutate(
    Condition = factor(grammaticality, levels = c("Grammatical", "Violation"))
  )

# Step 4: Ensure the "plots/" Directory Exists
if (!dir.exists("plots")) {
  dir.create("plots")
}

# Step 5: Dynamically Match Colors and Fills to Levels in the Data
available_levels <- levels(plot_data$Condition)
color_mapping <- c("Grammatical" = "forestgreen", "Violation" = "firebrick1")
fill_mapping <- c("Grammatical" = "forestgreen", "Violation" = "firebrick1")

# Filter color and fill mappings to match available levels
filtered_color_mapping <- color_mapping[available_levels]
filtered_fill_mapping <- fill_mapping[available_levels]

# Step 6: Generate and Save Plots for Each Brain Region
for (region in unique(plot_data$lab_IDs)) {
  # Filter data for the current brain region
  region_data <- plot_data %>% filter(lab_IDs == region)
  
  # Ensure there is more than one observation per group
  if (nrow(region_data) <= 1) {
    message(paste("Skipping plot for region:", region, "- Not enough data points"))
    next
  }
  
  # Create the plot
  plot <- ggplot(region_data, aes(x = grammaticality, y = mean_amplitude, color = Condition)) +
    geom_point(size = 4) +  # Use points for single observations
    ...
    scale_fill_manual(values = filtered_fill_mapping) +
    labs(
      title = paste("Brain Region:", region),
      x = "Condition",
      y = "Amplitude (μV)"
    ) +
    theme_minimal()
  ...
  # Save the plot
  ggsave(
    filename = paste0("plots/region_", region, ".png"),
    plot = plot,
    width = 10,
    height = 8,
    dpi = 300
